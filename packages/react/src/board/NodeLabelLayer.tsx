...
import type { RoomMorph } from '@cavin/core'
import { useCavin } from '../context'
import { paletteMemo } from '../presentation'
...
/**
 * The node's only on-graph form: a dot (DotsCanvas) plus this label,
...
export function NodeLabelLayer() {
  ...
  useEffect(() => {
    ...
    const paletteOf = paletteMemo(adapter)

    // Imperative hover-preview bubble (label + body + tags), built once.
    ...
    const tick = () => {
      ...
      const { cam, zRooms, zCards, timeT, selectedId, labelFont } = useView.getState()
      const world = useWorld.getState().world
      const palette = paletteOf(world)
      ...
      const syncContent = (el: HTMLElement, n: CavinNode, kids: number) => {
        const label = labelOf(adapter, n)
        const sig = `${n.state.locked ? 1 : 0}|${label}|${kids}|${n.state.placed ? 1 : 0}`
        if (sigById.get(n.id) === sig) return
        sigById.set(n.id, sig)
        el.replaceChildren()
        if (n.state.locked) el.append(document.createTextNode('🔒 '))
      ...
      // the center dot.
      let header: Label<unknown> | undefined
      let headerDepth = -1
      if (!focus) {
      ...
      // Recency tiebreak rides the adapter's time axis when it declares one.
      ...
      candidates.sort(
        (a, b) =>
          ...
          timeKey(b.node) - timeKey(a.node) ||
          a.node.id.localeCompare(b.node.id),
      )

      const box = (l: Label<unknown>): Placed => {
        const kids = world.childrenByParent.get(l.node.id)?.length ?? 0
        const cw = estimateWidth(labelOf(adapter, l.node), kids, fontScale)
        if (l.place === 'below') {
      ...
      for (const l of candidates) {
        ...
        insert(rect)
        labels.push(l)
        if (labels.length >= LABELS.maxChips) break
      }
      ...
      for (const l of labels) {
        ...
        if (!el) {
          ...
          el.style.borderColor = palette.nodeColor(l.node)
          el.style.pointerEvents = 'auto'
          elById.set(l.node.id, el)
          layer.append(el)
        }
        const kids = world.childrenByParent.get(l.node.id)?.length ?? 0
        const tf = palette.timeDim(l.node, timeT)
        const selected = l.node.id === selectedId
        const related = focus != null && !selected
        const cls = `chip${l.place === 'below' ? ' child' : ''}${l.room ? ' room' : ''}${l === header ? ' header' : ''}${selected ? ' selected' : ''}${related ? ' focus-label' : ''}${l.node.state.locked ? ' locked' : ''}${drag.dragging && drag.nodeId === l.node.id ? ' dragging' : ''}${drag.dragging && drag.targetId === l.node.id ? ' drop-target' : ''}${shakeId === l.node.id ? ' shake' : ''}`
        applyClass(el, cls)
      ...
      if (showPreview && hoverId) {
        ...
        if (n && pos) {
          if (previewFilled !== hoverId) {
            ...
            previewTitle.textContent = labelOf(adapter, n)
            previewBody.textContent = bodyOf(adapter, n)
            previewTags.replaceChildren(
              ...tagsOf(adapter, n).map((t) => {
                const s = document.createElement('span')
                s.className = 'tag'
                s.textContent = t
                return s
          ...
          preview.style.borderColor = palette.nodeColor(n)
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      const mpos = posById.get(id) ?? useWorld.getState().world.nodeById.get(id)?.position ?? [0, 0]
    ...
    const hitTestDropTarget = (clientX: number, clientY: number): string | null => {
      const { cam, zCards } = useView.getState()
      const world = useWorld.getState().world
      const w = window.innerWidth
      const h = window.innerHeight
      const morph = new Map<string, RoomMorph>()
      ...
      for (const n of world.nodes) {
        if (n.id === drag.nodeId || n.state.locked || drag.descendants?.has(n.id)) continue
        const m = roomMorph(world, n, cam.zoom, cam, zCards, morph)
        if (n.parentId && !n.state.placed && m.open < 0.3) continue // folded away — invisible
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (!drag.dragging) {
        ...
        const node = useWorld.getState().world.nodeById.get(drag.nodeId)
        if (!node) {
          drag.pointerId = 0
          return
        }
        if (node.state.locked) {
          ...
          drag.dragging = false
          shake(drag.nodeId)
          useWorld.getState().toast(`${cap(noun)} is locked — unlock it first`)
          return
        }
        drag.dragging = true
        drag.moved = true
      ...
      const zoom = useView.getState().cam.zoom
      drag.pos = [drag.pos[0] + (e.clientX - drag.lastX) / zoom, drag.pos[1] + (e.clientY - drag.lastY) / zoom]
      drag.lastX = e.clientX
      drag.lastY = e.clientY
      useWorld.getState().moveNode(drag.nodeId, drag.pos)
    ...
    const endDrag = (e: PointerEvent) => {
      ...
      if (targetId && targetId !== nodeId) {
        useWorld.getState().reparentNode(nodeId, targetId)
      } else if (drag.moved) {
        ...
        const node = useWorld.getState().world.nodeById.get(nodeId)
        if (node?.parentId) useWorld.getState().promoteNode(nodeId)
      }
    }
    ...
    const onClick = (e: MouseEvent) => {
      ...
      useView.getState().select(el.dataset.id!)
    }
    ...
    const onDoubleClick = (e: MouseEvent) => {
      ...
      e.stopPropagation()
      const id = el.dataset.id!
      const node = useWorld.getState().world.nodeById.get(id)
      if (!node) return
      if (node.state.locked) {
        shake(id)
        useWorld.getState().toast(`${cap(noun)} is locked — unlock it first`)
      ...
      useView.getState().select(id)
      useWorld.getState().setEditing(id)
    }
