...
import { focusForSelection, layoutConfig, revealZoom, visibleNodes } from '@cavin/core'
import type { RoomMorph } from '@cavin/core'
import { useCavin } from '../context'
import { MAX_ZOOM, MIN_ZOOM } from '../view-store'
...
export function Board() {
      ...
      if (s.cam !== lastCrumbCam) {
        ...
        if (now - lastCrumbAt > 200) {
          ...
          if (crumb !== lastBreadcrumb) {
            ...
            useView.getState().setBreadcrumb(crumb)
          }
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      const worldStore = useWorld.getState()
      if (e.key === 'Escape') {
        view.select(null)
  ...
  useEffect(() => {
    ...
    if (!node) return
    const { cam, zRooms, zCards } = useView.getState()
  ...
  const onPointerUp = () => {
    if (drag.current && !drag.current.moved) {
      ...
      useWorld.getState().setEditing(null)
    }
    drag.current = null
  }
  ...
  const onDoubleClick = (e: ReactMouseEvent<HTMLDivElement>) => {
    ...
    const px = e.clientX - rect.left - rect.width / 2
    const py = e.clientY - rect.top - rect.height / 2
    useWorld.getState().addNode([cam.x + px / cam.zoom, cam.y + py / cam.zoom])
  }
